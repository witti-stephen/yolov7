...
class ONNX_ORT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.non_concat_final:
            ...
            return selected_scores.reshape(1,-1), (selected_boxes/640).unsqueeze(-1).permute(2,0,1), selected_categories.reshape(1,-1), selected_indices.shape[0].unsqueeze(-1).float() # tflite required 4 return in such sequence
        else:
            return torch.cat([X, selected_boxes, selected_categories, selected_scores], 1)
        # [End Update End2End to include --non-concat-final]
